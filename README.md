...
一个零依赖的抖音无水印视频下载工具，支持两种下载模式，提供三种使用方式：

- **Next.js + Vercel 版（推荐）**：一键部署到 Vercel 免费版，无需维护服务器，全球可访问
- **浏览器插件版**：Chrome/Edge MV3 扩展，在抖音页面注入下载按钮，不依赖任何服务器（见下方「方式三」）
...
- **图集支持**：图文作品自动识别，预览全部无水印原图，一键逐张下载（手机端友好，无需解压）
...
`images[].download_url_list[0]`，最高清原图），一键「下载全部图片」**逐张保存**
...
## 方式二：浏览器插件版（Chrome / Edge MV3）

在抖音视频/图集页右下角注入「无水印下载」按钮，工具栏弹窗可粘贴链接解析下载。
**不依赖任何服务器**——用 `declarativeNetRequest` 直接给抖音 CDN 注入 `Referer`
后由浏览器直连，无 web 版的 10s 超时和带宽限制。
...
然后在 `chrome://extensions` 开启「开发者模式」→「加载已解压的扩展程序」→
选择 `extension/` 目录。详见 [`extension/README.md`](./extension/README.md)。
